...
export const MetricCard = memo<MetricCardProps>(({ metric }) => {
    const renderTrend = () => {
        if (metric.trendDirection === 'UP') {
            return (
                <span className="flex items-center text-[var(--color-success)] text-sm font-medium">
        ...
        if (metric.trendDirection === 'DOWN') {
            return (
                <span className="flex items-center text-[var(--color-error)] text-sm font-medium">
        ...
        return (
            <span className="flex items-center text-[var(--color-text-muted)] text-sm font-medium">
    ...
    return (
        <div className="card p-5 flex flex-col gap-2 transition-all hover:shadow-lg">
            <h3 className="text-[var(--color-text-muted)] text-sm font-medium">{metric.label}</h3>
            <div className="flex items-end justify-between mt-1">
                <div className="flex items-baseline gap-1">
                    <span className="text-2xl font-bold text-[var(--color-text)]">{metric.value}</span>
                    {metric.unit && <span className="text-sm text-[var(--color-text-muted)] font-medium">{metric.unit}</span>}
                </div>
                {renderTrend()}
            </div>
        </div>
